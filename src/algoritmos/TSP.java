...
public class TSP {
    ...
    public Rota otimizarRota(Grafo grafo, Ponto origem) {
        return vizinhoMaisProximo(grafo, origem);
    }

    public Rota otimizarRotaComDestinoFinal(Grafo grafo, Ponto destinoFinal) {
        if (grafo == null || destinoFinal == null) {
            throw new IllegalArgumentException("Grafo e destino final sao obrigatorios.");
        }

        List<Ponto> percurso = new ArrayList<>();
        Set<Ponto> visitados = new HashSet<>();
        Ponto atual = destinoFinal;
        percurso.add(atual);
        visitados.add(atual);
        ...
        return new Rota(percurso, null, destinoFinal, percurso.isEmpty() ? null : percurso.get(percurso.size() - 1),
                "Caixeiro Viajante", distancia, AlgoritmoUtils.calcularTempo(distancia),
                distancia, AlgoritmoUtils.calcularCusto(distancia));
    }

    private Rota vizinhoMaisProximo(Grafo grafo, Ponto origem) {
